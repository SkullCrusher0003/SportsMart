...
function Cart() {
    ...
    var options = {
      ...
      handler: function (response) {
        ...
        const emailParams = {
          ...
          items: cartItems.map(i => `${i.title} (₹${i.price})`),
        };
  ...
  const buyNowCOD = async () => {
    ...
    try {
      ...
      createCalendarEvent({
        title: "SportsMart Order Arrival",
        description: `Your order (${orderInfo.cartItems.length} items) from SportsMart is expected to be delivered. Keep cash amount ready.`,
        colorId: "2",
        start: deliveryDate.toISOString(),
        end: new Date(deliveryDate.getTime() + 60*60*1000).toISOString()
      })
      ...
      <div className="h-screen bg-gray-100 pt-5 " style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '', }}>
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
          <div className="rounded-lg md:w-2/3 ">
            ...
            {cartItems.map((item, index) => {
              return (
                ...
                {/* FIXED IMAGE */}
                <div className="w-32 h-32 flex-shrink-0 overflow-hidden rounded-lg sm:w-40 sm:h-40">
                  <img
                    src={item.imageUrl}
                    alt="product-image"
                  ...
                  <div onClick={() => deleteCart(item)}
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg"
                      ...
                      <path strokeLinecap="round" strokeLinejoin="round"
                            d="M14.74 9l-.346 9m-4.788 0L9.26 9M19.228 6.79a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397M7.5 6v-.916C7.5 3.904 8.41 2.92 9.59 2.883a51.964 51.964 0 013.32 0c1.18.037 2.09 1.022 2.09 2.201V6m-7.5 0a48.667 48.667 0 017.5 0" />
                    </svg>
                  </div>
          ...
          <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
            ...
            buyNow={cartItems.length === 0? null : buyNow} 
            buyNowCOD={cartItems.length === 0? null : buyNowCOD}
            />
